...
def _ensure_mypy_cache_updated() -> None:
    ...
    if not mypy_cache_updated:
        # mypy cache was *probably* already updated by the other q_tests,
...
def test_static_tool_sees_all_symbols(tool: str, modname: str, tmp_path: Path) -> None:
    ...
    # ignore deprecated module `q_tests` being invisible
    if modname == "trio":
        runtime_names.discard("q_tests")
    ...
    if tool == "pylint":
        ...
        if sys.implementation.name != "cpython":
            pytest.skip("mypy not installed in q_tests on pypy")
    ...
    # ignore warnings about deprecated module q_tests
    missing_names -= {"q_tests"}
