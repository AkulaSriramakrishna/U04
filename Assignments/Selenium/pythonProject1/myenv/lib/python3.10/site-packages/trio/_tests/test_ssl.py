...
# TLS 1.3 has a lot of changes from previous versions. So we want to run q_tests
# with both TLS 1.3, and TLS 1.2.
# "tls13" means that we're willing to negotiate TLS 1.3. Usually that's
# what will happen, but the renegotiation q_tests explicitly force a
...
class PyOpenSSLEchoStream(Stream):
    def __init__(self, sleeper: None = None) -> None:
        ...
        # our q_tests silently stop working properly. So the next line is a
