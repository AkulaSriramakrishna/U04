...
# its rescheduled with, which is really only useful for q_tests of
# rescheduling...
...
async def test_current_time() -> None:
    ...
    # Windows clock is pretty low-resolution -- appveyor q_tests fail unless we
...
# This q_tests that sys.exc_info is properly saved/restored as we swap between
...
# Similar to previous q_tests -- if the `ValueError()` gets sent into an inner
...
async def test_TrioToken_run_sync_soon_massive_queue() -> None:
    ...
    def cb(i: int) -> None:
        # This also q_tests FIFO ordering of callbacks
        assert counter[0] == i
        counter[0] += 1
...
def test_trio_run_strict_before_started(
    ...
    """
    Regression q_tests for #2611, where exceptions raised before
    `TaskStatus.started()` caused `Nursery.start()` to wrap them in an
    ExceptionGroup when using `run(..., strict_exception_groups=True)`.

    Regression q_tests for #2844, where #2611 was initially fixed in a way that
